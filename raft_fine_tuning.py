...
class EpochProgressCallback(TrainerCallback):
    """Custom callback to print epoch-level progress"""
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        if state.log_history:
            ...
            logger.info(
                f"  Progress: {epoch}/{total_epochs} ({100*epoch/total_epochs:.1f}%)")
            logger.info(f"{'='*80}\n")
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if logs and 'loss' in logs:
            ...
            logger.info(
                f"📊 Step {state.global_step} | Epoch {epoch:.2f} | Loss: {loss:.4f}")
...
class RAFTTrainer:
    ...
    def train(self):
        ...
        allow_cpu = bool(self.config.get(
            'hardware', {}).get('allow_CPU', False))
        if not torch.cuda.is_available() and not allow_cpu:
            raise RuntimeError(
                "CUDA GPU not available and hardware.allow_CPU is False. Aborting.")
        ...
        trainer = Trainer(
            ...
            callbacks=[
                ...
                EpochProgressCallback()  # Add custom progress logging
            ]
        )
